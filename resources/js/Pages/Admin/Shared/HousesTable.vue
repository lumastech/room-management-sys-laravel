        ...
        <table class="table-auto w-full">
            ...
            <tbody>
                <tr v-for="house in houses.data" :key="house.id" class="border-b text-gray-400 hover:bg-teal-50">
                    ...
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.name }}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.type }}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.area }}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.town }}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{house.rooms}}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.walfance}}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.water}}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{house.electricity}}</Link></td>
                    <td class="truncate py-2 pr-2"><Link :href="`/admin/house/show/${house.id}`" class="w-full block">{{ house.car_park}}</Link></td>
                    ...
                    <td class="text-right">
                        <button :id="'tdrop_'+house.id" :data-dropdown-toggle="'tdrop_down_'+house.id" class="text-teal-500" type="button"><svg class="w-6 h-6" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>
                        <!-- Dropdown menu -->
                        <div :id="'tdrop_down_'+house.id" class="z-10 text-left hidden bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700">
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" :aria-labelledby="'tdrop_'+house.id">
                                <li>
                                    <Link :href="'/admin/house/update/'+house.id" method="put" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full text-left" ><i class="fas fa-edit mr-2"></i>Edit</Link>
                                </li>
                                <li>
                                    <button @click="modalShow(`${'/admin/house/destroy/'+house.id}`, 'delete', 'Are you sure you want to delete this property?')" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full text-left" ><i class="fas fa-trash-can mr-2"></i> Delete</button>
                                </li>
                            </ul>
                        </div>
